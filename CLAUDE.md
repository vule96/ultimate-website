...
> **🩺 Issue tracker (2026-07-11):** Senior code review toàn codebase (Go core B+, admin B+, web B) — findings đầy đủ ở **`docs/reviews/2026-07-11-senior-code-review.md`**. Đây là **nơi note & track mọi issue**: khi resolve issue nào phải đánh dấu `✅ RESOLVED (ngày, commit)` ngay tại finding đó trong file; issue mới phát hiện cũng note vào đây. **Slice 5a DONE: C1, H1, H4, A1, A4, M7. Slice 5b DONE: W1, W2, H2, H3, A2, A3 (+M6, L7).** Không còn việc khẩn (high+). **Đợt 3 (polish, chưa làm):** W3 (sitemap/rss throw), W4–W6 (SEO tag metadata/JSON-LD/OG + not-found/error pages), W7 (sanitize + CSP), `next/font`, TS tightening admin (A5–A8), M1–M5 core, outbox (chuẩn bị Phase 2).
>
> **📍 Điểm hiện tại (2026-07-11):** **Phase 1 HOÀN TẤT** (Slice 1→4) + **Slice 5a + 5b DONE** — mọi issue high+ từ senior review đã resolved (C1/H1/H4/A1/A4/M7 + W1/W2/H2/H3/A2/A3/M6/L7). Blog công khai verify live; storage R2 verify (dev=MinIO/prod=R2). **Bước kế tiếp CHƯA chốt** — **Slice 5c (đợt 3 polish:** SEO tag metadata/JSON-LD/OG, not-found/error pages, sanitize+CSP, `next/font`, TS tightening admin, outbox**)** *hoặc* **Deploy production** *hoặc* **Phase 2 (AI)**; khi chốt → brainstorm → spec. Roadmap: Artifact "Status & Roadmap" (`https://claude.ai/code/artifact/78ac518f-fc25-44d0-9385-d85dfbfde92e`).
