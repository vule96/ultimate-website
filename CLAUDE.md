...
> **🩺 Issue tracker (2026-07-11):** Senior code review toàn codebase (Go core B+, admin B+, web B) — findings đầy đủ ở **`docs/reviews/2026-07-11-senior-code-review.md`**. Đây là **nơi note & track mọi issue**: khi resolve issue nào phải đánh dấu `✅ RESOLVED (ngày, commit)` ngay tại finding đó trong file; issue mới phát hiện cũng note vào đây. **Slice 5a (đợt 1) DONE (2026-07-11): C1, H1, H4, A1, A4, M7 resolved.** Việc khẩn còn lại: **W1** (pagination query-string hỏng ở production web — SSG route trả sai trang). Đợt 2/3 còn: A2/A3 (401-aware admin), H2/H3 (graceful shutdown + logging), path pagination, SEO, sanitize/CSP, outbox.
>
> **📍 Điểm hiện tại (2026-07-11):** **Phase 1 HOÀN TẤT** (Slice 1→4) + **Slice 5a (Security & Data-loss Hardening) DONE** — resolved C1/H1/H4/A1/A4/M7 (chi tiết ở issue tracker). Blog công khai đã **verify chạy live**; storage **R2 verify** end-to-end (dev = MinIO / prod = R2). **Bước kế tiếp CHƯA chốt** — Slice 5b/5c (đợt 2/3 hardening: W1 pagination, 401-aware admin, shutdown/logging, SEO...) *hoặc* **Deploy production** *hoặc* **Phase 2 (AI)**; khi chốt → brainstorm → spec. Roadmap trực quan: Artifact "Status & Roadmap" (`https://claude.ai/code/artifact/78ac518f-fc25-44d0-9385-d85dfbfde92e`).
