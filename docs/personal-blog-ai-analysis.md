...
| Routing | **React Router** | Client-side routing (thay App Router của Next). |
...
│   └── admin/          # React SPA (Vite + React Router) — dashboard quản trị (mục 14)
├── packages/
...
**`apps/admin` (React SPA — Vite + React Router):**

```
apps/admin/src/
├── main.tsx               # entry SPA
├── router.tsx             # React Router (routes + protected route)
├── features/              # nhóm theo tính năng
│   ├── posts/{api,components,hooks,types}.ts
│   ├── editor/  auth/  media/
├── components/ui/         # shadcn (import từ packages/ui)
├── lib/                   # api client (JWT), utils, config
└── hooks/  types/  styles/
```

Tham khảo kiến trúc chuẩn: [bulletproof-react](https://github.com/alan2207/bulletproof-react). Nguyên tắc: **colocation** (code liên quan ở gần nhau), tính năng độc lập, `features/*` không import chéo lộn xộn. Khác biệt chính: `web` route bằng App Router (file-based), `admin` route bằng React Router (khai báo trong `router.tsx`).

---

## 14. Admin Dashboard — **App React SPA riêng (đã chốt)**

**Quyết định (cập nhật 2026-07-05):** Admin dashboard là **app React SPA riêng (`apps/admin`, Vite + React + React Router)** trong cùng monorepo — **KHÔNG dùng Next.js**. Next.js chỉ dành cho `apps/web` (blog công khai cần SEO/SSR).
...
| **Học SPA đúng nghĩa** | Client routing (React Router), quản lý auth token, protected route — kỹ năng SPA cổ điển. |
...
- **Routing:** React Router (khai báo trong `router.tsx`), không có file-based routing như Next.
...
- **`apps/admin` (React SPA Vite):** build ra **static** → deploy cực nhẹ trên **Cloudflare Pages / Vercel (static)** (không cần Node runtime SSR), hoặc phục vụ static qua Coolify/Nginx trên VPS. Nhớ cấu hình **SPA fallback** (mọi route → `index.html`) cho React Router.
...
- Vite — https://vite.dev · React Router — https://reactrouter.com (cho `apps/admin` SPA)
