...
> **Tiến độ (2026-07-22):** **5.1 DONE** (L1–L6 resolved, commit `d4458c8` — xem tracker). **5.2 DONE** (FK cascade test: harness thêm FK ON DELETE CASCADE sau AutoMigrate + `TestFK_ReaderDeleteCascadesBookmarks`/`TestFK_PostDeleteCascadesBookmarks` raw DELETE). **5.3 BLOCKED** — cần R2 credentials thật (`.env.prod STORAGE_*` còn placeholder `<account>`), thao tác tay của bạn.
