...
> **✅ DONE (2026-07-20, branch `backlog-admin-part3`):** 3.1, 3.2, 3.3, 3.4, 3.5, 3.6, 3.8. Bỏ 3.7 (dark mode — vẫn light-only theo yêu cầu). Chi tiết + verify từng mục ghi ngay dưới. Spec: `docs/superpowers/specs/2026-07-20-backlog-admin-part3-design.md`.
...
### 3.7 Dark mode admin (nếu muốn — hiện light-only)  ⏭️ SKIP (giữ light-only theo yêu cầu)
...
> **Tiến độ (2026-07-23):** **5.1 DONE** (L1–L6 resolved, commit `d4458c8` — xem tracker). **5.2 DONE** (FK cascade test: harness thêm FK ON DELETE CASCADE sau AutoMigrate + `TestFK_ReaderDeleteCascadesBookmarks`/`TestFK_PostDeleteCascadesBookmarks` raw DELETE). **5.3 DONE** — R2 credentials thật đã điền `.env.prod`; verify E2E: `backup-db.sh` (pg_dump→gzip→upload `ultimate-backups`, list OK 8620B) + restore drill từ R2 vào DB scratch `blog_restore_drill` (posts=12, subscribers=2 khớp). **Toàn bộ Backlog (5) DONE.**
>
> ⚠️ **2 lưu ý:** (a) bucket `ultimate-media` (ảnh) token hiện **403** — R2 API token chưa scope tới nó; upload ảnh prod chưa chạy tới khi sửa scope token gồm cả `ultimate-media` (không cản backup). (b) `scripts/backup-db.sh`/`restore-db.sh` dùng `mktemp -d` (path MSYS) → Docker Desktop **Windows local không mount được**; script **đúng cho VPS Linux** (đích thật). Verify local phải dùng path Windows (`C:/...`).
