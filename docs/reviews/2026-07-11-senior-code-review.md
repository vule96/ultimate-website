...
- L12 *(phát hiện khi làm Slice 5d, 2026-07-12)*: test integration core dùng chung dev DB (`TEST_DATABASE_URL`) — seed data thủ công (E2E/verify) làm các test đếm toàn bảng (Stats/List total) fail chập chờn dù tx-rollback. Fix gợi ý: DB test riêng (vd `blog_test`) hoặc test đếm theo delta/filter thay vì count tuyệt đối.
