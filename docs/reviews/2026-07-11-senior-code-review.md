...
>
> **✅ Slice 5c — Polish FE (đợt 3, nhóm A) HOÀN TẤT (2026-07-11).** Resolved: W3, W4, W5, W6, W7 + low SEO (OG fallback, RSS enrich, cover sizes, next.config gate) + font `next/font` + A5, A6, A7, A8, A9 + admin low (tagKeys, PostId, apiClient 204/rename, toast role, aria-sort, clamp page, Topbar link). Spec/plan `...slice5c-polish-fe...`.
>
> **✅ Slice 5d — Backend Robustness (đợt 4, nhóm B) HOÀN TẤT (2026-07-12).** Resolved: M1–M5 core (tag ON CONFLICT, session allowlist recheck, route shadowing, body-size limit, optimistic locking) + transactional outbox (chuẩn bị Phase 2).
>
> **✅ Slice 5e — Dọn backlog Low HOÀN TẤT (2026-07-12).** Resolved: L8, L9, L10, L11, L12. Toàn bộ finding Critical/High/Medium/Low đã được yêu cầu resolve trong senior review đợt 2026-07-11 giờ **DONE**; còn mở duy nhất **L1–L6** (low, chưa được yêu cầu ưu tiên — xem mục tương ứng ở trên). Spec: `docs/superpowers/specs/2026-07-12-slice5e-low-backlog-design.md` · Plan: `docs/superpowers/plans/2026-07-12-slice5e-low-backlog.md`.
...
- L10: tags mồ côi không bao giờ được dọn → inflate `/tags` + `Stats.Tags`. ✅ **RESOLVED (2026-07-12, commit 24a75d6 + b39c7a5)** — dọn orphan tags trong cùng transaction ngay sau update/delete post, scope theo đúng tag vừa gỡ khỏi bài (không quét/khoá toàn bảng `tags`), dùng `FOR UPDATE` khi kiểm tra tag còn được bài khác dùng không. Lưu ý: residual race ở mức READ COMMITTED với nhiều writer đồng thời (tx A tái sử dụng tag đang bị tx B xoá) là lý thuyết, chấp nhận được cho single-admin hiện tại (đã ghi trong comment code) — cần xem lại khi Phase 2+ có nhiều writer (AI worker) ghi post đồng thời.
- L11: `getBoolEnv` nuốt lỗi parse (`config.go:80-90`) — `SESSION_COOKIE_SECURE=ture` fail im lặng; nên trả error từ `Load`. ✅ **RESOLVED (2026-07-12, commit 8437f48)** — `getBoolEnv`/`getInt64Env` trả `error`, `Load` fail-fast khi env rác. Verify live: `SESSION_COOKIE_SECURE=ture` → `go run ./cmd/api` panic ngay: `config: SESSION_COOKIE_SECURE must be a boolean (true/false), got "ture"` (không listen, không treo).
- L12 *(phát hiện khi làm Slice 5d, 2026-07-12)*: test integration core dùng chung dev DB (`TEST_DATABASE_URL`) — seed data thủ công (E2E/verify) làm các test đếm toàn bảng (Stats/List total) fail chập chờn dù tx-rollback. Cùng lớp: 2 test dispatcher outbox (`outbox_test.go` `TestDispatcher_ProcessesAndMarks`/`HandlerErrorKeepsEvent`) query pending không filter — có thể dính row committed từ session khác. Fix gợi ý: DB test riêng (vd `blog_test`), hoặc test đếm theo delta/filter và scope query dispatcher-test theo event id. ✅ **RESOLVED (2026-07-12, commit d860b15 + 653ef6f)** — DB test riêng `blog_test` tự tạo qua `postgres-init` (cách li khỏi dev DB); test đếm chuyển sang so delta/token, dispatcher-test scope theo event id/aggregate. Verify: full suite `go test ./...` (env `TEST_DATABASE_URL` trỏ `blog_test`) xanh toàn bộ, không skip module nào ngoài dự kiến.
- L13 *(phát hiện khi làm Slice 13, 2026-07-18 — final whole-branch review)*: **FK `ON DELETE CASCADE` của `bookmarks` (reader→readers, post→posts) chưa có test.** Test integration `readers` dùng GORM `AutoMigrate` (không tạo FK — FK chỉ nằm trong migration Atlas `add_readers.sql`), nên hành vi cascade khi xoá post/reader chưa được khoá bằng test + có drift model↔migration. Fix gợi ý: hoặc thêm FK tag vào GORM model, hoặc 1 test integration chạy trên DB đã apply migration thật để pin cascade. *(chưa làm — low, để backlog)*
- L14 *(phát hiện khi làm Slice 13, 2026-07-18)*: **rate limit + view dedupe theo `ClientIP()` chỉ đúng khi `TRUSTED_PROXIES` cấu hình khớp reverse proxy prod.** Đã wire `SetTrustedProxies` qua env (rỗng = dùng IP kết nối trực tiếp, an toàn cho dev), nhưng nếu deploy sau Nginx/Cloudflare mà quên set `TRUSTED_PROXIES` thì `X-Forwarded-For` bị bỏ qua → mọi client trông như 1 IP (proxy) → rate limit gộp sai. **Launch-blocker cho slice deploy VPS**: set `TRUSTED_PROXIES` = IP/CIDR proxy trước khi mở public. *(note deploy — không phải bug code)*
- L15 *(phát hiện khi làm Slice 13, 2026-07-18)*: **chưa có unsubscribe flow + reader profile/xoá tài khoản (GDPR).** Newsletter chỉ subscribe (`subscribers.status` có cột `unsubscribed` nhưng chưa có endpoint/link token); reader không xoá được tài khoản/bookmark của mình. Để Phase 2 (khi AI worker gửi mail thật → cần unsubscribe link). *(chưa làm — low, để backlog)*
...
- ✅ **RESOLVED (2026-07-18, commit ecb6d68)** `reading-progress.tsx` — bỏ setState mỗi scroll + `transition-[width]` (giật, "chỉ nhảy sau khi scroll xong") → `useRef` + `requestAnimationFrame` gộp scroll thành ≤1 ghi/frame + `transform: scaleX()` GPU-composited (không re-render React). Dày 2px→4px, màu `bg-accent` (Mạch). Verify live playwright: top→scaleX(0), mid→scaleX(0.5), bottom→scaleX(1) mượt từng frame.
- ✅ **RESOLVED (2026-07-18, commit ecb6d68) — A10 (admin đồng bộ):** phát hiện `apps/admin` (+`apps/web`) hay bị bỏ quên khi làm BE/web → thêm quy ước bắt buộc trong `CLAUDE.md` (Quy ước làm việc): mỗi tính năng chạm API/schema/`packages/*` phải `tsc --noEmit && build` admin + đưa vào verify; lưu ý image Docker cũ không tự phản ánh code mới (phải rebuild). Root cause sự cố 2026-07-18: stack Docker prod chạy trên localhost với `.env.prod` (`CORS_ALLOWED_ORIGINS=https://admin.example.com`, `SECURE=true`) → core reject `localhost:5173` → admin `_authed` beforeLoad (đúng thiết kế, rethrow non-401) hiện error boundary. KHÔNG phải bug admin code (build sạch). Fix môi trường local: `.env.prod` (gitignored) trỏ localhost + recreate/rebuild stack.
